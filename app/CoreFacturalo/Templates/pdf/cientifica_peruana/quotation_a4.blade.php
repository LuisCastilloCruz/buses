...
<table class="full-width mt-1 mb-1">
    ...
    <tr class="bg-grey- fondo-ficho">
        ...
        <th class="text-left text-white py-2" width="8%" style="background: {{$color2}}">MODELO</th>
        ...
        <tr>
            ...
            <td class="text-left">
                @if($row->item->name_product_pdf ?? false)
                    <span style="font-size: 8.5px"> {!!$row->item->name_product_pdf ?? ''!!} </span>
                @else
                    <span style="font-size: 8.5px"> {!!$row->item->description!!}  </span>
                @endif
                @if (!empty($row->item->presentation)) {!!$row->item->presentation->description!!} @endif
                @if($row->attributes)
                    @foreach($row->attributes as $attr)
                        <br/><span style="font-size: 8px">{!! $attr->description !!} : {{ $attr->value }}</span>
                    @endforeach
                @endif
                @if($row->discounts)
                    @foreach($row->discounts as $dtos)
                        <br/><span style="font-size: 8px">{{ $dtos->factor * 100 }}% {{$dtos->description }}</span>
                    @endforeach
                @endif
