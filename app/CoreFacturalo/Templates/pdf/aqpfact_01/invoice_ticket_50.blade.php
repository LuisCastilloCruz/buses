...
@if($company->logo)
    <div class="text-center pt-2 desc-9">
        <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->name}}" class=" content" style="max-height: 100px">
    </div>
{{--@else--}}
...
<table class="full-width">
    <tr>
        <td class="text-center"><h5 style="font-size: 11px"><b>{{ $company->name }}</b></h5></td>
    </tr>
    {{--<tr>
    ...
    <tr>
        <td class="text-center "><span style="font-size: 10px">{{  ($establishment->trade_address !== '-')? 'D. Comercial: '.$establishment->trade_address : ''  }}</span></td>
    </tr>
    @endisset
    <tr>
        <td class="text-center "><span style="font-size: 10px">{{ ($establishment->telephone !== '-')? 'Central telefónica: '.$establishment->telephone : '' }}</span></td>
    </tr>
    <tr>
        <td class="text-center"><span style="font-size: 10px">{{ ($establishment->email !== '-')? 'Email: '.$establishment->email : '' }}</span></td>
    </tr>
    @isset($establishment->web_address)
        <tr>
            <td class="text-center"><span style="font-size: 10px">{{ ($establishment->web_address !== '-')? 'Web: '.$establishment->web_address : '' }}</span></td>
        </tr>
    @endisset

    @isset($establishment->aditional_information)
        <tr>
            <td class="text-center pb-3"><span style="font-size: 10px">{{ ($establishment->aditional_information !== '-')? $establishment->aditional_information : '' }}</span></td>
        </tr>
    @endisset
    ...
    <tr>
        <td class="text-center pb-3 border-bottom"><h5><b>{{ $document_number }}</b></h5></td>
    </tr>
</table>
<table class="full-width">
    <tr>
        <td width="" class="pt-3"><p class="desc-9"><b>F. Emisión</b>:</p></td>
        <td width="" class="pt-3 "><p class="desc-9">{{ $document->date_of_issue->format('Y-m-d') }}</p></td>
    </tr>

    @isset($invoice->date_of_due)
    <tr>
        <td><p class="desc-9"><b>F. Vencimiento:</b></p></td>
        <td><p class="desc-9">{{ $invoice->date_of_due->format('Y-m-d') }}</p></td>
    </tr>
    @endisset

    <tr>
        <td class="align-top"><p class="desc-9"><b>Cliente:</b></p></td>
        <td><p class="desc-9">{{ $customer->name }}</p></td>
    </tr>
    <tr>
        <td><p class="desc-9"><b>{{ $customer->identity_document_type->description }}:</b></p></td>
        <td><p class="desc-9">{{ $customer->number }}</p></td>
    </tr>
    @if ($customer->address !== '')
        <tr>
            <td class="align-top"><p class="desc-9"><b>Dirección:</b></p></td>
            <td>
                <p class="desc-9">
                    {{ $customer->address }}
    ...
    @if ($document->quotation_id)
        <tr>
            <td><p class="desc-9"><b>Cotización:</b></p></td>
    ...
    @isset($document->quotation->sale_opportunity)
        <tr>
            <td><p class="desc-9"><b>O. Venta</b></p></td>
...
<table class="full-width">
    <tr>
        ...
        @foreach(array_reverse((array) $document->legends) as $row)
            <tr>
                @if ($row->code == "1000")
                    <td class="desc-9 pt-1">Son: <span class="font-bold">{{ $row->value }} {{ $document->currency_type->description }}</span></td>
                    @if (count((array) $document->legends)>1)
                    <tr><td class="desc-9 pt-1"><span class="font-bold">Leyendas</span></td></tr>
                    @endif
                @else
                    <td class="desc-9 pt-1">{{$row->code}}: {{ $row->value }}</td>
                @endif
            </tr>
        @endforeach
    </tr>
    ...
    <tr>
        <td class="text-center company_logo_box_sm"><img class="content" width="100" src="data:image/png;base64, {{ $document->qr }}" width="100"/></td>
    </tr>
    <tr>
    ...
    @if($document->payment_method_type_id)
        <tr>
            <td class="desc pt-5">
                <strong>PAGO: </strong>{{ $paymentCondition }}
            </td>
        </tr>
    @endif
    @if ($document->payment_condition_id === '01')
