...
<table class="full-width mt-3 client-aqp">
   <tr>
       <td width="120px"><b>FECHA DE EMISIÓN</b></td>
       <td width="8px">:</td>
   ...
   @if ($document->detraction)
       <td width="140px"><b>B/S SUJETO A DETRACCIÓN</b></td>
       <td width="8px">:</td>
    ...
    @endif
    <tr>
        <td style="vertical-align: top;"><b>CLIENTE:</b></td>
...
<table class="full-width mt-10 mb-10 datos-empresa">
    ...
    <tr class="bg-grey- fondo-ficho">
        <th class="text-center text-white py-2" width="6%" style="background: {{$color1}}">CANT.</th>
        <th class="text-center text-white py-2" width="6%" style="background: {{$color1}}">UND.</th>
        <th class="text-center text-white py-2" width="7%" style="background: {{$color1}}">COD.</th>
        <th class="text-left text-white py-2" style="background: {{$color1}}">DESCRIPCIÓN</th>
        <th class="text-center text-white py-2" width="7%" style="background: {{$color1}}">LOTE</th>
        <th class="text-center text-white py-2" width="6%" style="background: {{$color2}}">SERIE</th>
        <th class="text-right text-white py-2" width="8%" style="background: {{$color2}}">V.UNIT</th>
        <th class="text-right text-white py-2" width="8%" style="background: {{$color2}}">P.UNIT</th>
        <th class="text-right text-white py-2" width="6%" style="background: {{$color2}}">DTO.</th>
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-center align-top borde-gris">{{ $row->item->internal_id}}</td>
            ...
            @if ($configuration_decimal_quantity->change_decimal_quantity_unit_price_pdf)
                <td class="text-right align-top borde-gris">{{ $row->generalApplyNumberFormat($row->unit_value, $configuration_decimal_quantity->decimal_quantity_unit_price_pdf) }}</td>
            @else
                <td class="text-right align-top borde-gris">{{ number_format($row->unit_value, 2) }}</td>
            @endif

            @if ($configuration_decimal_quantity->change_decimal_quantity_unit_price_pdf)
                <td class="text-right align-top borde-gris">{{ $row->generalApplyNumberFormat($row->unit_price, $configuration_decimal_quantity->decimal_quantity_unit_price_pdf) }}</td>
            @else
        ...
        <table class="full-width">
            <tr>
                ...
                @foreach($payments as $row)
                    <tr>
                        <td>&#8226; {{ $row->payment_method_type->description }} - {{ $row->reference ? $row->reference.' - ':'' }} {{ $document->currency_type->symbol }} {{ number_format( ($row->payment + $row->change),2) }}</td>
                    </tr>
                @endforeach
            </tr>
        </table>
...
@else
    <table class="full-width">
            @foreach($document->fee as $key => $quote)
                <tr>
                    <td>&#8226; {{ (empty($quote->getStringPaymentMethodType()) ? 'Cuota #'.( $key + 1) : $quote->getStringPaymentMethodType()) }} / Fecha: {{ $quote->date->format('d-m-Y') }} / Monto: {{ $quote->currency_type->symbol }}{{ number_format(($quote->amount),2) }}</td>
                </tr>
            @endforeach
        </tr>
    </table>
