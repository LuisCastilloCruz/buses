...
<table class="full-width">
    ...
    <tr>
        <td class="text-center pb-3 border-bottom"><b style="font-size: 20px">{{ $tittle }}</b></td>
    </tr>
</table>
<table class="full-width">
    <tr>
        <td class="pt-3"><p class="desc"><b>F. Emisión:</b></p></td>
    ...
    @if ($document->due_date)
        <tr>
            ...
            <td width="" class="pt-3" colspan="3"><p class="desc">{{ $document->getFormatDueDate() }}</p></td>
        </tr>
    @endif
</table>
...
<table class="full-width mt-10 mb-10">
    ...
    @endforeach
        @if($document->total_exportation > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">OP. EXPORTACIÓN: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_exportation, 2) }}</td>
            </tr>
        @endif
        @if($document->total_free > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">OP. GRATUITAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_free, 2) }}</td>
            </tr>
        @endif
        @if($document->total_unaffected > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">OP. INAFECTAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_unaffected, 2) }}</td>
            </tr>
        @endif
        @if($document->total_exonerated > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">OP. EXONERADAS: {{ $document->currency_type->symbol }}</td>
        ...
        @if($document->total_charge > 0 && $document->charges)
            <tr>
                <td colspan="3" class="text-right font-bold desc">CARGOS ({{$document->getTotalFactor()}}%): {{ $document->currency_type->symbol }}</td>
    ...
    @if (!is_null($pasaje))
        ...
        <tr>
            <td class="text-center desc pb-2">
               <b style="font-size: 40px">{{ $pasaje->estado_pago_id == '1' ? 'PAGADO' : 'PAGO EN DESTINO' }}</b>
            </td>
        </tr>
    @endif
    @if (!is_null($encomienda))
